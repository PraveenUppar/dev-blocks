...
import { getPublishedPostService, getPublishedPostByIdService, getPublishedPostBySlugService, getUserClerkIdService, createPostService, editPostService, deletePostService, publishPostService, toggleLikePostService, toggleBookmarkPostService,
...
export async function createPostController(req, res, next) {
    try {
        ...
        const post = await createPostService(user.id, title, content, subtitle, coverImage);
...
export async function editPostController(req, res, next) {
    try {
        ...
        const { title, subtitle, content, coverImage } = req.body;
        const post = await editPostService(postId, title, content, subtitle, coverImage);
        return res.json({ success: true, data: post });
    }
    catch (error) {
        next(error);
